...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        // Dimension of new picture with border
        int w2 = w + 2 * borderWidth;
        int h2 = h + 2 * borderWidth;

        Pixel[][] src = pic.getBitmap();
        Pixel[][] tgt = new Pixel[h2][w2];

        if (borderWidth > 0) {
            for (int row = 0; row < h2; row++) {
                for (int col = 0; col < w2; col++) {
                    if (row < borderWidth || row > h2 - 1 - borderWidth ||
                            col < borderWidth || col > w2 - 1 - borderWidth) {
    ...
    public static PixelPicture scaleColors(
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                int gr = (int) Math.round(gfactor * g);
                int br = (int) Math.round(bfactor * b);
                bmp[row][col] = new Pixel(sr, gr, br);
            }
        }
        return new PixelPicture(bmp);
    }
        ...
        if (!(w1 == w2 && h1 == h2)) {
            ...
            for (int col = 0; col < w1; col++) {
                for (int row = 0; row < h1; row++) {
                    ...
                    int b1 = p1.getBlue();
                    int b2 = p2.getBlue();
                    bmp1[row][col] = new Pixel(weightedAverage(alpha, r1, r2),
                                               weightedAverage(alpha, g1, g2),
                                               weightedAverage(alpha, b1, b2));
                }
            }
